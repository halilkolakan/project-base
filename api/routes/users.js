  ...
  try {
    ...
    for (let i = 0; i < roles.length; i++) {
      await UserRoles.create({
        role_id: roles[i]._id,
        user_id: user._id,
        permission: roles[i].permission || "user",
        created_by: user._id,
      });
    }
...
router.post("/update", async (req, res) => {
  try {
    ...
    if (Array.isArray(body.role) && body.roles.length > 0) {
      let userRoles = await userRoles.find({ user_id: body._id });

      let removeRoles = userRoles.filter(
        (x) => !body.role.includes(x.role_id.toString())
      );
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    await UserRoles.create({
      ...
      created_by: createdUser._id,
      permission: Enum.PERMISSIONS.FULL, // ya da "full"
    });
...
router.post("/auth", async (req, res) => {
  try {
    ...
    let payload = {
      id: user._id,
      exp: parseInt(Date.now() / 100) * config.JWT.EXPIRE_TIME,
    };
    ...
    res.json(Response.successResponse({ token, user: userDate }));
  } catch (err) {
    let errorResponse = Response.errorResponse(err);
    res.status(errorResponse.code).json(errorResponse);
  }
